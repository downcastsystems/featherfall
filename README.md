...
- Enter starts the match from the lobby. Up/down and Enter navigate the title, pause, and results menus. At least two participants are required; bots count.
...
6. Press **Start** again to ready. The match starts when all joined participants are ready. The controller-accessible Take flight option or keyboard Enter can also launch.

During play, left/right moves, **A** taps or holds to flap, **down** dives straight down, **X** boosts in the direction you face, and **Start** pauses/resumes. On the title, pause, and results screens, use **up/down** and **A** to select options. **B** returns or resumes. In the lobby, **B** or Back leaves. Disconnecting a participating controller pauses the game; reconnect it and press Start to resume. New participants join between matches.
